...
private:
    ...
    void InitImGui()
    {
        ...
        if (VK_SUCCESS != vkQueueSubmit(m_graphicsQueue, 1, &end_info, VK_NULL_HANDLE))
        {
            throw std::runtime_error("Failed to submit Imgui queue");
        }
        vkDeviceWaitIdle(m_device);
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/06_02_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/06_02_base_frag.spv");
    ...
    void CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) const
    {
        ...
        // 提交到内存传输指令队列执行内存传输
        if (VK_SUCCESS != vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, nullptr))
        {
            throw std::runtime_error("Failed to submit memory transfer queue");
        }
