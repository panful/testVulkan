/**
 * 1. Descriptor 的基础使用
 * 2. 多个 DescriptorSet 的使用，对应着色器中的 layout(set = XX, binding = 0)
 */
...
private:
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/01_03_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/01_03_base_frag.spv");
...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            vkDestroyBuffer(m_device, m_colorUniformBuffers.at(i), nullptr);
            vkFreeMemory(m_device, m_colorUniformBuffersMemory.at(i), nullptr);
            vkDestroyBuffer(m_device, m_mvpUniformBuffers.at(i), nullptr);
            vkFreeMemory(m_device, m_mvpUniformBuffersMemory.at(i), nullptr);
        }

        vkDestroyDescriptorPool(m_device, m_descriptorPool, nullptr);
        vkDestroyDescriptorSetLayout(m_device, m_colorDescriptorSetLayout, nullptr);
...
private:
    ...
    void CreateGraphicsPipeline()
    {
        auto vertShaderCode = ReadFile("../resources/shaders/01_03_base2_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/01_03_base2_frag.spv");
        ...
        std::array<VkDescriptorSetLayout, 2> descriptorSetLayouts {m_colorDescriptorSetLayout, m_mvpDescriptorSetLayout};
    ...
    {
        {
            ...
            if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &m_colorDescriptorSetLayout))
            {
                throw std::runtime_error("failed to create descriptor set layout");
            }
        }

        {
            ...
            if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &m_mvpDescriptorSetLayout))
            {
                throw std::runtime_error("failed to create descriptor set layout");
            }
        }
    ...
    {
        {
            VkDeviceSize bufferSize = sizeof(UniformBufferObjectColor);

            m_colorUniformBuffers.resize(MAX_FRAMES_IN_FLIGHT);
            m_colorUniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
            m_colorUniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);
            ...
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
            {
                CreateBuffer(
                    ...
                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    m_colorUniformBuffers.at(i),
                    m_colorUniformBuffersMemory.at(i)
                );
                vkMapMemory(m_device, m_colorUniformBuffersMemory.at(i), 0, bufferSize, 0, &m_colorUniformBuffersMapped.at(i));
            }
        }

        {
            VkDeviceSize bufferSize = sizeof(UniformBufferObjectMVP);

            m_mvpUniformBuffers.resize(MAX_FRAMES_IN_FLIGHT);
            m_mvpUniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
            m_mvpUniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);
            ...
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
            {
                CreateBuffer(
                    ...
                    VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    m_mvpUniformBuffers.at(i),
                    m_mvpUniformBuffersMemory.at(i)
                );
                vkMapMemory(m_device, m_mvpUniformBuffersMemory.at(i), 0, bufferSize, 0, &m_mvpUniformBuffersMapped.at(i));
            }
        }
    }
    ...
    void UpdateUniformBuffer(uint32_t currentImage)
    {
        ...
        mvp.view  = glm::lookAt(glm::vec3(0.f, 0.f, -2.f), glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f));
        mvp.proj  = glm::perspective(glm::radians(45.f), aspect, 0.1f, 100.f);

        // glm的裁剪坐标的Y轴和 Vulkan 是相反的
        mvp.proj[1][1] *= -1;

        // 将变换矩阵的数据复制到uniform缓冲
        std::memcpy(m_mvpUniformBuffersMapped[currentImage], &mvp, sizeof(mvp));

        UniformBufferObjectColor color {
            .color = {1.f, 0.f, 1.f}
        };

        std::memcpy(m_colorUniformBuffersMapped[currentImage], &color, sizeof(color));
    }
    ...
    void CreateDescriptorSets()
    {
        ...
        layouts.insert(layouts.cend(), MAX_FRAMES_IN_FLIGHT, m_colorDescriptorSetLayout);
        layouts.insert(layouts.cend(), MAX_FRAMES_IN_FLIGHT, m_mvpDescriptorSetLayout);
        ...
        // 描述符集对象会在描述符池对象清除时自动被清除
        m_descriptorSets.resize(MAX_FRAMES_IN_FLIGHT * 2); // 并行两帧 * (color + mvp)
        if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &allocInfo, m_descriptorSets.data()))
        {
            throw std::runtime_error("failed to allocate descriptor sets");
        }
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            colorBufferInfo.buffer = m_colorUniformBuffers[i];
            colorBufferInfo.offset = 0;
            colorBufferInfo.range  = sizeof(UniformBufferObjectColor);
            ...
            descriptorWrites[0].dstSet           = m_descriptorSets.at(i); // 指定要更新的描述符集对象
            descriptorWrites[0].dstBinding       = 0;                      // 指定缓冲绑定
            descriptorWrites[0].dstArrayElement  = 0;                      // 描述符数组的第一个元素的索引（没有数组就使用0）
            descriptorWrites[0].descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[0].descriptorCount  = 1;
            descriptorWrites[0].pBufferInfo      = &colorBufferInfo;       // 指定描述符引用的缓冲数据
            descriptorWrites[0].pImageInfo       = nullptr;                // 指定描述符引用的图像数据
            descriptorWrites[0].pTexelBufferView = nullptr;                // 指定描述符引用的缓冲视图

            descriptorWrites[1].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[1].dstSet           = m_descriptorSets.at(i + 2); // 前两个是 color 后两个是 mvp
            descriptorWrites[1].dstBinding       = 0;
            descriptorWrites[1].dstArrayElement  = 0;
            descriptorWrites[1].descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[1].descriptorCount  = 1;
            descriptorWrites[1].pBufferInfo      = &mvpBufferInfo;
...
private:
    ...
    VkDescriptorSetLayout m_colorDescriptorSetLayout {nullptr};
    VkDescriptorSetLayout m_mvpDescriptorSetLayout {nullptr};

    std::vector<VkBuffer> m_colorUniformBuffers {};
    std::vector<VkDeviceMemory> m_colorUniformBuffersMemory {};
    std::vector<void*> m_colorUniformBuffersMapped {};

    std::vector<VkBuffer> m_mvpUniformBuffers {};
    std::vector<VkDeviceMemory> m_mvpUniformBuffersMemory {};
