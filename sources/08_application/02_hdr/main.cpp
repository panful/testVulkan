...
constexpr VkFormat hdrFormat = VK_FORMAT_R32G32B32A32_SFLOAT;
...
struct PushConstant
{
    int enableHDR {1};    // 是否开启HDR
    int type {0};         // 处理HDR的方式
    float exposure {1.f}; // 曝光度（Manual Exposure 方式才会使用到）
};
...
private:
    ...
    void CreateImageDatasOffscreen(Offscreen& offscreen)
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            offscreen.colors[i].imageView = CreateImageView(offscreen.colors[i].image, hdrFormat, VK_IMAGE_ASPECT_COLOR_BIT);
        }
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/08_02_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/08_02_base_frag.spv");
    ...
    void CreateRenderPassOffscreen(Offscreen& offscreen)
    {
        ...
        colorAttachment.format                  = hdrFormat;                    // 颜色缓冲附着的格式
    ...
    void PrepareImGui()
    {
        ...
        m_pushConstant.enableHDR = enableHDR ? 1 : 0;
        static const char* const hdrTypes[] {"Manual Exposure", "Reinhard Tone Mapping"};
        ImGui::Combo("HDR type", &m_pushConstant.type, hdrTypes, 2);
        ImGui::InputFloat("exposure", &m_pushConstant.exposure, 0.01f);
        ImGui::End();
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/08_02_hdr2ldr_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/08_02_hdr2ldr_frag.spv");
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        vkCmdPushConstants(commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PushConstant), &m_pushConstant);
