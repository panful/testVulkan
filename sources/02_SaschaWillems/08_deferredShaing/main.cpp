...
private:
    ...
    void CreateAttachmentImpl(VkFormat format, VkImageUsageFlags usage, VkImage& image, VkDeviceMemory& memory, VkImageView& view)
    {
        ...
        CreateImage(m_swapChainExtent.width, m_swapChainExtent.height, format, VK_IMAGE_TILING_OPTIMAL, usage, /*not use*/ 0, image, memory, true);
    ...
    {
        m_attachmentImageColor.resize(m_swapChainImageViews.size());
        m_attachmentMemoryColor.resize(m_swapChainImageViews.size());
        m_attachmentViewColor.resize(m_swapChainImageViews.size());

        m_attachmentImageDepth.resize(m_swapChainImageViews.size());
        m_attachmentMemoryDepth.resize(m_swapChainImageViews.size());
        m_attachmentViewDepth.resize(m_swapChainImageViews.size());

        m_attachmentImageNormal.resize(m_swapChainImageViews.size());
        m_attachmentMemoryNormal.resize(m_swapChainImageViews.size());
        m_attachmentViewNormal.resize(m_swapChainImageViews.size());

        m_attachmentImagePosition.resize(m_swapChainImageViews.size());
        m_attachmentMemoryPosition.resize(m_swapChainImageViews.size());
        m_attachmentViewPosition.resize(m_swapChainImageViews.size());

        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            CreateAttachmentImpl(m_swapChainImageFormat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, m_attachmentImageColor[i], m_attachmentMemoryColor[i],
                m_attachmentViewColor[i]);

            CreateAttachmentImpl(VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, m_attachmentImageNormal[i],
                m_attachmentMemoryNormal[i], m_attachmentViewNormal[i]);

            CreateAttachmentImpl(VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, m_attachmentImagePosition[i],
                m_attachmentMemoryPosition[i], m_attachmentViewPosition[i]);

            CreateAttachmentImpl(FindDepthFormat(), VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, m_attachmentImageDepth[i],
                m_attachmentMemoryDepth[i], m_attachmentViewDepth[i]);
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_08_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_08_base_frag.spv");
        ...
        std::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments(3, colorBlendAttachment);
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_08_quad_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_08_quad_frag.spv");
        ...
        pipelineInfo.renderPass                   = m_renderPass;
        pipelineInfo.subpass                      = 1;       // 子流程在子流程数组中的索引
        pipelineInfo.basePipelineHandle           = nullptr; // 以一个创建好的图形管线为基础创建一个新的图形管线
        pipelineInfo.basePipelineIndex            = -1; // 只有该结构体的成员 flags 被设置为 VK_PIPELINE_CREATE_DERIVATIVE_BIT 才有效
        pipelineInfo.pDepthStencilState           = &depthStencil;
    ...
    void CreateRenderPass()
    {
        ...
        colorAttachment.loadOp                  = VK_ATTACHMENT_LOAD_OP_CLEAR;              // 渲染之前对附着中的颜色和深度进行操作
        colorAttachment.storeOp                 = VK_ATTACHMENT_STORE_OP_DONT_CARE;         // 渲染之后对附着中的颜色和深度进行操作
        ...
        colorReference.attachment            = 1; // 颜色附件，帧缓冲第一个附件是颜色附件
        ...
        depthReference.attachment            = 4; // 深度附件
        depthReference.layout                = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

        std::array<VkAttachmentReference, 3> colorRefs { colorReference, normalReference, positionReference };

        subpassDescriptions[0].pipelineBindPoint       = VK_PIPELINE_BIND_POINT_GRAPHICS;
        subpassDescriptions[0].colorAttachmentCount    = static_cast<uint32_t>(colorRefs.size());
        subpassDescriptions[0].pColorAttachments       = colorRefs.data();
        subpassDescriptions[0].pDepthStencilAttachment = &depthReference;

        //--------------------------------------------------------------------------
        // 第二个子流程
        VkAttachmentReference swapChainReference = {};
        swapChainReference.attachment            = 0; // 帧缓冲第0个附件是交换链
        ...
        std::array<VkAttachmentDescription, 5> attachments { swapChainAttachment, colorAttachment, normalAttachment, positionAttachment,
            depthAttachment };
    ...
    void CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            ...
            std::array<VkImageView, 5> attachments { m_swapChainImageViews[i], m_attachmentViewColor[i], m_attachmentViewNormal[i],
                m_attachmentViewPosition[i], m_attachmentViewDepth[i] };
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        clearValues.at(1).color = { { 0.f, 0.f, 0.f, 0.f } };
        clearValues.at(2).color = { { 0.f, 0.f, 0.f, 0.f } };
        clearValues.at(3).color = { { 0.f, 0.f, 0.f, 0.f } };
        clearValues.at(4).depthStencil = { 1.f, 0 }; // Vulkan 的深度范围是 [0.0, 1.0] 1.0对应视锥体的远平面，初始化时应该设置为远平面的值
    ...
    void RecreateSwapChain()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            colorImageInfo.imageView   = m_attachmentViewColor[i];
            colorImageInfo.sampler     = nullptr; // 输入附件不需要采样器，仅仅只是像素本地加载不需要采样

            VkDescriptorImageInfo normalImageInfo {};
            normalImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            normalImageInfo.imageView   = m_attachmentViewNormal[i];
            normalImageInfo.sampler     = nullptr;

            VkDescriptorImageInfo positionImageInfo {};
            positionImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            positionImageInfo.imageView   = m_attachmentViewPosition[i];
            positionImageInfo.sampler     = nullptr;
            ...
            descriptorWrites[0].descriptorType   = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT; // 输入附件
            descriptorWrites[0].descriptorCount  = 1;
            descriptorWrites[0].pBufferInfo      = nullptr;         // 指定描述符引用的缓冲数据
            descriptorWrites[0].pImageInfo       = &colorImageInfo; // 指定描述符引用的图像数据
            descriptorWrites[0].pTexelBufferView = nullptr;         // 指定描述符引用的缓冲视图

            // 法线附件
            descriptorWrites[1].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[1].dstSet           = m_descriptorSetsQuad[i];
            ...
            descriptorWrites[3].dstSet           = m_descriptorSetsQuad[i];
    ...
    void CleanupSwapChain() noexcept
    {
        ...
        for (size_t i = 0; i < m_attachmentImageColor.size(); ++i)
        {
            vkDestroyImage(m_device, m_attachmentImageColor[i], nullptr);
            vkDestroyImage(m_device, m_attachmentImageNormal[i], nullptr);
            vkDestroyImage(m_device, m_attachmentImagePosition[i], nullptr);
            vkDestroyImage(m_device, m_attachmentImageDepth[i], nullptr);

            vkFreeMemory(m_device, m_attachmentMemoryColor[i], nullptr);
            vkFreeMemory(m_device, m_attachmentMemoryNormal[i], nullptr);
            vkFreeMemory(m_device, m_attachmentMemoryPosition[i], nullptr);
            vkFreeMemory(m_device, m_attachmentMemoryDepth[i], nullptr);

            vkDestroyImageView(m_device, m_attachmentViewColor[i], nullptr);
            vkDestroyImageView(m_device, m_attachmentViewNormal[i], nullptr);
            vkDestroyImageView(m_device, m_attachmentViewPosition[i], nullptr);
            vkDestroyImageView(m_device, m_attachmentViewDepth[i], nullptr);
        }
    ...
    void CreateDescriptorPool()
    {
        ...
        poolSizes[2].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * 4); // color、normal、position、depth
    ...
    void CreateTextureImage()
    {
        ...
        TransitionImageLayout(
            m_textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
        // 执行图像数据复制操作
    ...
    void CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
        VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, bool attachment = false)

    {
        ...
        imageInfo.usage         = attachment ? usage | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT : usage;
    ...
    /// @param newLayout
    /// @param aspect
    void TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkImageAspectFlags aspect) const
    {
        VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
        ...
        else if (VK_IMAGE_LAYOUT_UNDEFINED == oldLayout && VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL == newLayout)
        {
            sourceStage      = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
            destinationStage = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;
        }
...
private:
    ...
    std::vector<VkImage> m_attachmentImageColor {};
    std::vector<VkDeviceMemory> m_attachmentMemoryColor {};
    std::vector<VkImageView> m_attachmentViewColor {};
    std::vector<VkImage> m_attachmentImageDepth {};
    std::vector<VkDeviceMemory> m_attachmentMemoryDepth {};
    std::vector<VkImageView> m_attachmentViewDepth {};
    std::vector<VkImage> m_attachmentImageNormal {};
    std::vector<VkDeviceMemory> m_attachmentMemoryNormal {};
    std::vector<VkImageView> m_attachmentViewNormal {};
    std::vector<VkImage> m_attachmentImagePosition {};
    std::vector<VkDeviceMemory> m_attachmentMemoryPosition {};
    std::vector<VkImageView> m_attachmentViewPosition {};
