 ...
 * 2. 变换 + ImGui
 * 3. 变换 + ImGui + Drawable
 * 4. 变换 + ImGui + Drawable + Texture
 * 5. 变换 + ImGui + Drawable + Offscreen
 */
...
private:
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_16_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_16_base_frag.spv");
...
private:
    ...
    void Cleanup() noexcept
    {
        ...
        auto destroyTexture = [this](Texture& texture) {
        ...
        destroyDrawable(m_drawablePlane);
        destroyTexture(m_texturePlane);
        vkDestroySampler(m_device, m_samplerPlane, nullptr);
...
private:
    void CreateDrawables()
    {
        ...
        CreateTexture(m_texturePlane, "../resources/textures/nightsky.png");
        CreateSampler(m_samplerPlane);
    }
    ...
    void CreateSampler(VkSampler& sampler)
    {
        ...
        if (VK_SUCCESS != vkCreateSampler(m_device, &samplerInfo, nullptr, &sampler))
        {
            throw std::runtime_error("failed to create texture sampler");
        }
    }

    void CreateTexture(Texture& texture, const std::string& fileName)
    {
    ...
    void CreateGraphicsPipeline()
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_16_texture_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_16_texture_frag.spv");
    ...
    void CreateDescriptorSets()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            imageInfo.imageView   = m_texturePlane.imageView;
            imageInfo.sampler     = m_samplerPlane;
...
private:
    ...
    Drawable m_drawablePlane {};
    Texture m_texturePlane {};
    VkSampler m_samplerPlane {};
...
private:
    ...
    void InitVulkan()
    {
        ...
        CreateImageDatasOffscreen(m_offscreen);
        CreateRenderPassOffscreen(m_offscreen);
        CreateDescriptorSetLayoutOffscreen(m_offscreen);
        CreateGraphicsPipelineOffscreen(m_offscreen);
        CreateFramebuffersOffscreen(m_offscreen);
        CreateUniformBuffersOffscreen(m_offscreen);
        CreateDescriptorSetsOffscreen(m_offscreen);
    ...
    void Cleanup() noexcept
    {
        ...
        auto destroyOffscreen = [this](Offscreen& offscreen) {
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
            {
                vkDestroyBuffer(m_device, offscreen.uniformBuffers.at(i), nullptr);
                vkFreeMemory(m_device, offscreen.uniformBuffersMemory.at(i), nullptr);

                vkDestroyImage(m_device, offscreen.colors[i].image, nullptr);
                vkFreeMemory(m_device, offscreen.colors[i].memory, nullptr);
                vkDestroyImageView(m_device, offscreen.colors[i].imageView, nullptr);

                vkDestroyFramebuffer(m_device, offscreen.framebuffers[i], nullptr);
            }

            vkDestroyImageView(m_device, offscreen.depth.imageView, nullptr);
            vkDestroyImage(m_device, offscreen.depth.image, nullptr);
            vkFreeMemory(m_device, offscreen.depth.memory, nullptr);

            vkDestroyPipeline(m_device, offscreen.pipeline, nullptr);
            vkDestroyPipelineLayout(m_device, offscreen.pipelineLayout, nullptr);
            vkDestroyRenderPass(m_device, offscreen.renderPass, nullptr);
            vkDestroyDescriptorSetLayout(m_device, offscreen.descriptorSetLayout, nullptr);
        };

        destroyDrawable(m_drawablePlane);
        destroyOffscreen(m_offscreen);
...
private:
    void CreateDrawables()
    {
        ...
        CreateSampler(m_offscreenSampler);
    }
    ...
    void CreateImageDatasOffscreen(Offscreen& offscreen)
    {
        offscreen.colors.resize(MAX_FRAMES_IN_FLIGHT);
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            CreateImage(
                m_swapChainExtent.width,
                ...
                0,
                offscreen.colors[i].image,
                offscreen.colors[i].memory
            );

            offscreen.colors[i].imageView = CreateImageView(offscreen.colors[i].image, m_swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
        }
        ...
        CreateImage(
            ...
            offscreen.depth.memory
        );

        offscreen.depth.imageView = CreateImageView(offscreen.depth.image, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
    }

    void CreateGraphicsPipelineOffscreen(Offscreen& offscreen)
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_16_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_16_base_frag.spv");
        ...
        pipelineLayoutInfo.sType                  = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pipelineLayoutInfo.setLayoutCount         = 1;
        pipelineLayoutInfo.pSetLayouts            = &offscreen.descriptorSetLayout;
        pipelineLayoutInfo.pushConstantRangeCount = 0;

        if (VK_SUCCESS != vkCreatePipelineLayout(m_device, &pipelineLayoutInfo, nullptr, &offscreen.pipelineLayout))
        {
            throw std::runtime_error("failed to create pipeline layout");
        }
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &offscreen.pipeline))
        {
            throw std::runtime_error("failed to create graphics pipeline");
        }
    ...
    void CreateRenderPassOffscreen(Offscreen& offscreen)
    {
        // 附着描述
        VkAttachmentDescription colorAttachment = {};
        colorAttachment.format                  = m_swapChainImageFormat;       // 颜色缓冲附着的格式
        ...
        if (VK_SUCCESS != vkCreateRenderPass(m_device, &renderPassInfo, nullptr, &offscreen.renderPass))
        {
            throw std::runtime_error("failed to create render pass");
        }
    }

    void CreateFramebuffersOffscreen(Offscreen& offscreen)
    {
        offscreen.framebuffers.resize(MAX_FRAMES_IN_FLIGHT);

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            std::array<VkImageView, 2> attachments {offscreen.colors[i].imageView, offscreen.depth.imageView};
            ...
            if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebufferInfo, nullptr, &offscreen.framebuffers[i]))
            {
                throw std::runtime_error("failed to create framebuffer");
            }
        }
    ...
    void CreateDescriptorSetLayoutOffscreen(Offscreen& offscreen)
    {
        VkDescriptorSetLayoutBinding uboLayoutBinding = {};
        uboLayoutBinding.binding                      = 0;
        uboLayoutBinding.descriptorType               = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        ...
        if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &offscreen.descriptorSetLayout))
        {
            throw std::runtime_error("failed to create descriptor set layout");
        }
    }

    void CreateUniformBuffersOffscreen(Offscreen& offscreen)
    {
        VkDeviceSize bufferSize = sizeof(UniformBufferObject);

        offscreen.uniformBuffers.resize(MAX_FRAMES_IN_FLIGHT);
        offscreen.uniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
        offscreen.uniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            CreateBuffer(
                ...
                VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                offscreen.uniformBuffers.at(i),
                offscreen.uniformBuffersMemory.at(i)
            );
            vkMapMemory(m_device, offscreen.uniformBuffersMemory.at(i), 0, bufferSize, 0, &offscreen.uniformBuffersMapped.at(i));
        }
    }

    void CreateDescriptorSetsOffscreen(Offscreen& offscreen)
    {
        std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, offscreen.descriptorSetLayout);
        ...
        // 描述符集对象会在描述符池对象清除时自动被清除
        offscreen.descriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
        if (VK_SUCCESS != vkAllocateDescriptorSets(m_device, &allocInfo, offscreen.descriptorSets.data()))
        {
            throw std::runtime_error("failed to allocate descriptor sets");
        }
        ...
        {
            VkDescriptorBufferInfo bufferInfo {};
            bufferInfo.buffer = offscreen.uniformBuffers.at(i);
            bufferInfo.offset = 0;
            bufferInfo.range  = sizeof(UniformBufferObject);
            ...
            descriptorWrites[0].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[0].dstSet           = offscreen.descriptorSets.at(i);
            descriptorWrites[0].dstBinding       = 0;
            descriptorWrites[0].dstArrayElement  = 0;
            descriptorWrites[0].descriptorType   = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[0].descriptorCount  = 1;
    ...
    void CreateGraphicsPipeline()
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_16_offscreen_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_16_offscreen_frag.spv");
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        VkRenderPassBeginInfo renderPassInfo = {};
        renderPassInfo.sType                 = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass            = m_offscreen.renderPass;
        renderPassInfo.framebuffer           = m_offscreen.framebuffers[m_currentFrame];
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen.pipeline);
        vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
        vkCmdSetViewport(commandBuffer, 0, 1, &viewport);

        vkCmdBindDescriptorSets(
            commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_offscreen.pipelineLayout, 0, 1, &m_offscreen.descriptorSets[m_currentFrame], 0, nullptr
        );
    ...
    void CleanupResoucesOffscreen(Offscreen& offscreen)
    {
        vkDestroyImageView(m_device, offscreen.depth.imageView, nullptr);
        vkDestroyImage(m_device, offscreen.depth.image, nullptr);
        vkFreeMemory(m_device, offscreen.depth.memory, nullptr);
    ...
    void RecreateSwapChain()
    {
        ...
        // 清理并重建离屏渲染资源
        CleanupResoucesOffscreen(m_offscreen);
        CreateImageDatasOffscreen(m_offscreen);
        CreateFramebuffersOffscreen(m_offscreen);
    ...
    void UpdateUniformBuffer(uint32_t currentImage)
    {
        ...
        // 将变换矩阵的数据复制到uniform缓冲
        std::memcpy(m_offscreen.uniformBuffersMapped[currentImage], &ubo, sizeof(ubo));
    }
    ...
    void UpdateDescriptorSets()
    {
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            VkDescriptorImageInfo imageInfo {};
            imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            imageInfo.imageView   = m_offscreen.colors[i].imageView;
