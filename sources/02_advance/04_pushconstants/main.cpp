/**
 * 1. PushConstant 基础使用
 * 2. 同一个 PipeLine 对于不同的 Drawcall 提交多次 PushConstant
 * 3. 顶点着色器片段着色器共用一个 PushConstant
 * 4. 顶点着色器和片段着色器都使用一个 PushConstant
 */
...
private:
    ...
    void CreateGraphicsPipeline()
    {
        ...
        // 管线布局，在着色器中使用 uniform 变量、push_constant 变量
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        // 提交push_constant数据
        float color = static_cast<float>((std::cos(glfwGetTime()) + 1.0) / 2.0);
        PushConstant pc {glm::vec3(color)};
...
// clang-format off
const std::vector<Vertex> vertices {
    {{-0.8f, -0.5f}, {1.f, 0.f, 0.f}},
    {{-0.8f,  0.5f}, {1.f, 0.f, 0.f}},
    {{-0.1f,  0.5f}, {0.f, 1.f, 0.f}},
    {{-0.1f, -0.5f}, {0.f, 1.f, 0.f}},
};

const std::vector<Vertex> vertices2 {
    {{ 0.1f, -0.5f}, {1.f, 0.f, 0.f}},
    {{ 0.1f,  0.5f}, {1.f, 0.f, 0.f}},
...
private:
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        //--------------------------------------------------
        // 第一次提交 PushConstant 绘制
        PushConstant pc {glm::vec3(1.f, 0.f, 0.f)};
        vkCmdPushConstants(commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &pc);
        ...
        //--------------------------------------------------
        // 第二次提交 PushConstant 绘制
        PushConstant pc2 {glm::vec3(0.f, 1.f, 0.f)};
        vkCmdPushConstants(commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstant), &pc2);
...
private:
    ...
    void CreateGraphicsPipeline()
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_04_base2_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_04_base2_frag.spv");
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        // 顶点和片段着色器使用的 PushConstant 一次性提交，着色器中只访问自己需要的数据即可
        PushConstant pc = {
            {1.f, 0.f, 0.f},
            {0.f, 1.f, 0.f}
        };
...
private:
    ...
    void CreateGraphicsPipeline()
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_04_base3_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_04_base3_frag.spv");
        ...
        // PushConstant 顶点着色器和片段着色器各自都有一个 PushConstant
        ...
        pushConstantRangeVert.size                = sizeof(PushConstantVert);

        VkPushConstantRange pushConstantRangeFrag = {};
        pushConstantRangeFrag.stageFlags          = VK_SHADER_STAGE_FRAGMENT_BIT;
        pushConstantRangeFrag.offset              = sizeof(PushConstantVert);
        pushConstantRangeFrag.size                = sizeof(PushConstantFrag);

        VkPushConstantRange pushConstantRanges[] = {pushConstantRangeVert, pushConstantRangeFrag};
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        // 顶点和片段着色器使用的 PushConstant 一次性提交，着色器中只访问自己需要的数据即可
        PushConstantVert pcVert = {
            {1.f, 0.f, 0.f}
        };
        PushConstantFrag pcFrag = {
            {0.f, 1.f, 0.f}
        };
        vkCmdPushConstants(commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstantVert), &pcVert);
        vkCmdPushConstants(
            commandBuffer, m_pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(PushConstantVert), sizeof(PushConstantFrag), &pcFrag
        );
