...
private:
    ...
    void CreateAttachments()
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            ...
            if (index > 0)
            {
                ...
                barrier.image                           = m_attachmentImageDepth[i];
                barrier.subresourceRange.aspectMask     = VK_IMAGE_ASPECT_DEPTH_BIT; // 设置布局变换影响的范围
                ...
                vkCmdPipelineBarrier(
                    commandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_06_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_06_base_frag.spv");
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_06_quad_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_06_quad_frag.spv");
    ...
    void CreateRenderPass()
    {
        ...
        colorReference.attachment            = 1; // 帧缓冲第0个附件是交换链，第1个是颜色附件，第2个是深度附件
        ...
        dependencies[0].srcSubpass    = VK_SUBPASS_EXTERNAL;                  // 渲染流程开始前的子流程
        dependencies[0].dstSubpass    = 0;                                    // 渲染流程结束后的子流程
        dependencies[0].srcStageMask  = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT; // 指定需要等待的管线阶段
        dependencies[0].dstStageMask  = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
        dependencies[0].srcAccessMask = VK_ACCESS_MEMORY_READ_BIT; // 指定子流程将进行的操作类型
        ...
        std::array<VkAttachmentDescription, 3> attachments { swapChainAttachment, colorAttachment, depthAttachment };
    ...
    void CreateFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_swapChainImageViews.size(); ++i)
        {
            ...
            std::array<VkImageView, 3> attachments { m_swapChainImageViews[i], m_attachmentViewColor[i], m_attachmentViewDepth[i] };
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        clearValues.at(1).color = { { .1f, .2f, .3f, 1.f } }; // 清除色，相当于背景色
        clearValues.at(2).depthStencil = { 1.f, 0 }; // Vulkan 的深度范围是 [0.0, 1.0] 1.0对应视锥体的远平面，初始化时应该设置为远平面的值
    ...
    void RecreateSwapChain()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            depthImageInfo.sampler     = nullptr; // 输入附件不需要采样器，仅仅只是像素本地加载不需要采样
    ...
    void CleanupSwapChain() noexcept
    {
        ...
        for (auto&& image : m_attachmentImageColor)
        {
            vkDestroyImage(m_device, image, nullptr);
        }
        for (auto&& memory : m_attachmentMemoryColor)
        {
            vkFreeMemory(m_device, memory, nullptr);
        }
        for (auto&& view : m_attachmentViewColor)
        {
            vkDestroyImageView(m_device, view, nullptr);
        }

        for (auto&& image : m_attachmentImageDepth)
        {
            vkDestroyImage(m_device, image, nullptr);
        }
        for (auto&& memory : m_attachmentMemoryDepth)
        {
            vkFreeMemory(m_device, memory, nullptr);
        }
        for (auto&& view : m_attachmentViewDepth)
        {
            vkDestroyImageView(m_device, view, nullptr);
        }
    ...
    void CreateDescriptorPool()
    {
        ...
        poolSizes[2].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT * 2); // color和depth，所以需要乘以2
