...
private:
    ...
    void CreateLogicalDevice()
    {
        ...
        VkPhysicalDeviceFeatures deviceFeatures = {};
        deviceFeatures.fillModeNonSolid         = VK_TRUE; // 开启才能使用线框（三角形）
        deviceFeatures.wideLines                = VK_TRUE; // 开启才能设置线宽
    ...
    {
        // 三角形填充
        CreateGraphicsPipeline(m_graphicsPipelines.at(0), VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_POLYGON_MODE_FILL);
        // 三角形不填充
        CreateGraphicsPipeline(m_graphicsPipelines.at(1), VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, VK_POLYGON_MODE_LINE);
        // 线段
        CreateGraphicsPipeline(m_graphicsPipelines.at(2), VK_PRIMITIVE_TOPOLOGY_LINE_LIST, VK_POLYGON_MODE_LINE);
    }
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_03_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_03_base_frag.spv");
        ...
        inputAssembly.topology                               = topo; // 指定绘制的图元类型：点、线、三角形
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline))
        {
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipelines.at(0));
        ...
        //------------------------------------------------------------------
        // 线框三角形
        viewport.x = width / 3.f;
        vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipelines.at(1));
        vkCmdSetLineWidth(commandBuffer, 5.f); // 需要将线宽设置为动态状态才能使用该指令动态设置线宽
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, &m_vertexBuffer, offsets);
        vkCmdBindIndexBuffer(commandBuffer, m_indexBuffer, 0, VK_INDEX_TYPE_UINT16);
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
        ...
        //------------------------------------------------------------------
        // 线框四边形
        viewport.x = width / 3.f * 2.f;
        vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipelines.at(2));
        vkCmdSetLineWidth(commandBuffer, 3.f); // 如果此处不设置线宽，将使用之前设置的线宽
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, &m_vertexBuffer2, offsets);
        vkCmdBindIndexBuffer(commandBuffer, m_indexBuffer2, 0, VK_INDEX_TYPE_UINT16);
    ...
    {
        VkDeviceSize bufferSize = sizeof(indices2.front()) * indices2.size();
        ...
        void* data { nullptr };
        vkMapMemory(m_device, stagingBufferMemory, 0, bufferSize, 0, &data);
        std::memcpy(data, indices2.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(m_device, stagingBufferMemory);
...
private:
    ...
    std::array<VkPipeline, 3> m_graphicsPipelines {};
