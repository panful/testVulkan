...
private:
    ...
    {
        auto vertShaderCode = ReadFile("../resources/shaders/02_21_base_vert.spv");
        auto fragShaderCode = ReadFile("../resources/shaders/02_21_base_frag.spv");
...
private:
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        uint32_t w = m_swapChainExtent.width;  // framebuffer 的宽度
        uint32_t h = m_swapChainExtent.height; // farmebuffer 的高度

        VkRenderPassBeginInfo renderPassInfo = {};
        renderPassInfo.sType                 = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass            = m_renderPass;
        renderPassInfo.framebuffer           = framebuffer;
        renderPassInfo.renderArea.offset     = {50, 50};
        renderPassInfo.renderArea.extent     = {w / 2 - 100, h / 2 - 100};
        ...
        viewport.x          = 50.f;
        viewport.y          = 50.f;
        viewport.width      = static_cast<float>(w / 2 - 100);
        viewport.height     = static_cast<float>(h / 2 - 100);
        viewport.minDepth   = 0.f;
        viewport.maxDepth   = 1.f;

        VkRect2D scissor = {};
        scissor.offset   = {50, 50};
        scissor.extent   = {w / 2 - 100, h / 2 - 100};
        ...
        //--------------------------------------------------------------------
        // 第二次 RenderPass
        renderPassInfo.renderArea.offset = {(int32_t)w / 2 + 50, (int32_t)h / 2 + 50};
        renderPassInfo.renderArea.extent = {w / 2 - 100, h / 2 - 100};
        viewport.x                       = static_cast<float>(w / 2 + 50);
        viewport.y                       = static_cast<float>(h / 2 + 50);
        viewport.width                   = static_cast<float>(w / 2 - 100);
        viewport.height                  = static_cast<float>(h / 2 - 100);
        scissor.offset                   = {int32_t(w) / 2 + 50, int32_t(h) / 2 + 50};
        scissor.extent                   = {w / 2 - 100, h / 2 - 100};
...
private:
    ...
    void RecordCommandBuffer(const VkCommandBuffer commandBuffer, const VkFramebuffer framebuffer) const
    {
        ...
        VkClearAttachment clearAttachments {VK_IMAGE_ASPECT_DEPTH_BIT, 1, clearValue};
        VkClearRect clearRects {
            {VkOffset2D {0, 0}, VkExtent2D {w, h}},
            0, 1
        };
        vkCmdClearAttachments(commandBuffer, 1, &clearAttachments, 1, &clearRects);
...
public:
    ...
    void SetDepthOutput(const ImageData& depth)
    {
        m_outputDepth = depth;
    }
    ...
    void SetDevice(const VkPhysicalDevice physicalDevice, const VkDevice device, const VkDescriptorPool descriptorPool)
    {
    ...
    void Execute(const VkCommandBuffer commandBuffer, const UniformBufferObject& ubo, const size_t frameIndex, const uint32_t imageIndex) const
    {
        UpdateUniformBuffer(frameIndex, ubo);
    ...
    void CreateFramebuffers()
    {
        ...
        {
            std::array<VkImageView, 2> attachments {m_outputColors[i].imageView, m_outputDepth.imageView};
            ...
            if (VK_SUCCESS != vkCreateFramebuffer(m_device, &framebufferInfo, nullptr, &m_framebuffers[i]))
            {
                throw std::runtime_error("failed to create framebuffer");
            }
        }
    ...
    void CreateGraphicsPipeline()
    {
        ...
        if (VK_SUCCESS != vkCreateGraphicsPipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline))
        {
            throw std::runtime_error("failed to create graphics pipeline");
        }
    ...
    void CreateUniformBuffers()
    {
        ...
        m_mvp.buffers.resize(MAX_FRAMES_IN_FLIGHT);
        m_mvp.deviceMemorys.resize(MAX_FRAMES_IN_FLIGHT);
        m_mvp.mapped.resize(MAX_FRAMES_IN_FLIGHT);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            vkMapMemory(m_device, m_mvp.deviceMemorys[i], 0, bufferSize, 0, &m_mvp.mapped[i]);
        }
    }
    ...
    void CreateDescriptorSets()
    {
        ...
        {
            VkDescriptorBufferInfo bufferInfo {};
            bufferInfo.buffer = m_mvp.buffers[i];
            bufferInfo.offset = 0;
            bufferInfo.range  = sizeof(UniformBufferObject);

            VkWriteDescriptorSet descriptorWrite {};
            descriptorWrite.sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrite.dstSet           = m_descriptorSets.at(i);
    ...
    void UpdateUniformBuffer(const size_t frameIndex, const UniformBufferObject& ubo) const noexcept
    {
        std::memcpy(m_mvp.mapped[frameIndex], &ubo, sizeof(ubo));
    }
...
public:
    ...
    void Execute(const VkCommandBuffer cmd, const UniformBufferObject& ubo, const size_t frameIndex, const uint32_t imageIndex)
    {
        for (const auto& [_, renderPass] : m_renderPasses)
        {
            renderPass.Execute(cmd, ubo, frameIndex, imageIndex);
        }
    }
...
private:
    ...
    void InitVulkan()
    {
        ...
        auto& presentPass = m_renderGraph.AddPass("present");

        presentPass.AddDrawable(m_drawableCube);
        presentPass.AddDrawable(m_drawablePlane);
        presentPass.SetDevice(m_physicalDevice, m_device, m_descriptorPool);
        presentPass.SetExtent(m_swapChainExtent);
        presentPass.SetColorFormat(m_swapChainImageFormat);
        presentPass.SetDepthFormat(m_depthFormat);
        presentPass.SetColorOutput(m_colors);
        presentPass.SetDepthOutput(m_depth);
...
private:
    ...
    void InitImGui()
    {
        ...
        ImGui_ImplVulkan_Init(&init_info, m_renderGraph.GetRenderPass("present").GetRenderPass());
    ...
    void CreateSwapChain()
    {
        ...
        m_colors.resize(m_minImageCount);
        std::vector<VkImage> swapChainImages;
        for (auto i = 0u; i < m_minImageCount; ++i)
        {
            swapChainImages.emplace_back(m_colors[i].image);
        }
        vkGetSwapchainImagesKHR(m_device, m_swapChain, &m_minImageCount, swapChainImages.data());
        for (auto i = 0u; i < m_minImageCount; ++i)
        {
    ...
    void CreateImageViews()
    {
        for (auto i = 0u; i < m_minImageCount; ++i)
        {
            ...
            if (VK_SUCCESS != vkCreateImageView(m_device, &createInfo, nullptr, &m_colors[i].imageView))
            {
                throw std::runtime_error("failed to create image views");
            }
        }
    ...
    void DrawFrame()
    {
        ...
        m_renderGraph.Execute(m_commandBuffers.at(m_currentFrame), ubo, m_currentFrame, imageIndex);
    ...
    void CreateDepthResources()
    {
        ...
        m_depth.imageView = CreateImageView(m_depth.image, m_depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
    }
