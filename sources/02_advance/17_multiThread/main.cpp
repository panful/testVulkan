/**
 * 1. 每个线程并行生成辅助命令缓冲区
 * 2. 子线程中执行 ReocrdCommand
 * 3. 子线程中执行 DrawFrame 孙子线程执行 RecordCommand
 * 4. 子线程中执行 QueuqSubmit RecordCommand
 *
 */
...
private:
    ...
    void DrawFrame()
    {
        ...
        std::thread([this, imageIndex]() { RecordCommandBuffer(m_commandBuffers.at(imageIndex), m_swapChainFramebuffers.at(imageIndex)); }).join();
...
private:
    ...
    void MainLoop()
    {
        ...
        {
            glfwPollEvents();
            std::cout << "while thread id: " << std::this_thread::get_id() << std::endl;
...
private:
    ...
    void DrawFrame()
    {
        ...
        std::thread([this, imageIndex]() {
            std::cout << "record thread id: " << std::this_thread::get_id() << std::endl;
...
private:
    ...
    void DrawFrame()
    {
        ...
        std::thread([this, &submitInfo]() {
            std::cout << "submit thread id: " << std::this_thread::get_id() << std::endl;
            if (VK_SUCCESS != vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, m_inFlightFences.at(m_currentFrame)))
            {
                throw std::runtime_error("failed to submit draw command buffer");
            }
